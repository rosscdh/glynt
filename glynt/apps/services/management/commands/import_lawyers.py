...
=======
                if i > 0:
                    username = slugify(u'%s-%s'% (r[1], r[2],))
                    try:
                        u, is_new = User.objects.get_or_create(username=username, first_name=r[1], last_name=r[2], email=r[6])
                    except IntegrityError:
                        username = slugify(u'%s-%s%s'% (r[1], r[2],r[6],))
                        u, is_new = User.objects.get_or_create(username=username, first_name=r[1], last_name=r[2], email=r[6])
                    print u
                    ...
                    l, lawyer_is_new = Lawyer.objects.get_or_create(user=u, bio=r[len(r)-1], role=role, data=data)
                    print l
                    f, is_new = Firm.objects.get_or_create(name=r[3])
                    if lawyer_is_new:
                        f.lawyers.add(u)
                        print "added %s to %s" % (l,f,)
>>>>>>> 77f00d1... working on importer
