    ...
    def process(self):
        ...
        if self.office_address is not None:
            for i,o in enumerate(self.offices):
                try:
                    office = self.firm.office_set.get(address=self.office_address)
                    office_is_new = False
                except Office.DoesNotExist,AttributeError:
                    # add office
                    office_data = {'phone': office_phone}
                    office, office_is_new = Office.objects.get_or_create(firm=self.firm, address=self.office_address, data=office_data)
