...
from views import CompanyProfileSetupView, FounderProfileView, CreateFounderView
...
urlpatterns = patterns('',
    ...
    url(r'^founder/setup/$', login_required(CreateFounderView.as_view()), name='setup_founder'),
    url(r'^founder/(?P<slug>.+)/$', login_required(FounderProfileView.as_view()), name='founder_profile'),
    # also no list for companies #url(r'^$', login_required(LawyerListView.as_view()), name='list'),
)