...
class FounderQuestionnaire1(BootstrapMixin, forms.Form):
    first_name = forms.CharField(help_text="", widget=forms.TextInput(attrs={'placeholder':'First name', 'tabindex':'1'}))
    last_name = forms.CharField(help_text="", widget=forms.TextInput(attrs={'placeholder':'Last name','tabindex':'2'}))
    website = forms.URLField(label="URL", help_text="", widget=forms.TextInput(attrs={'placeholder':'http://acmeco.com', 'class':'input-large', 'tabindex':'3'}))
    agree_tandc = forms.BooleanField(
        label=mark_safe('I agree to the LawPal <a href="/legal/terms/" target="_blank">Terms and Conditions</a>?'),
                                     widget=forms.CheckboxInput(attrs={'tabindex':'9'}))


class FounderQuestionnaire2(BootstrapMixin, forms.Form):
    company_name = forms.CharField(label="Company name", help_text="", widget=forms.TextInput(attrs={'placeholder':'Acme Inc', 'tabindex':'1'}))
